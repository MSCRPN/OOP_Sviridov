...
const regex ContactValidator::PHONE_REGEX(R"((?:\+?[\s\-\(\)]*[78][\s\-\(\)]*)?(?:[\s\-\(\)]*\d[\s\-\(\)]*){10,11})");
...
const regex ContactValidator::EMAIL_REGEX(R"(^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$)");
    ...
    string ContactValidator::normalizeEmail(std::string s) {
    s = trim(s);

    QString q = QString::fromStdString(s).trimmed();
    q.replace(QRegularExpression(R"(\s*@\s*)"), "@");

    QRegularExpression md(R"(^\s*\[([^\]]+)\]\(mailto:[^)]+\)\s*$)");
    auto m = md.match(q);
    if (m.hasMatch()) {
        q = m.captured(1);
    }
...
bool ContactValidator::validateEmail(const std::string& email) {
    std::string e = normalizeEmail(email);
    return std::regex_match(e, EMAIL_REGEX);
}
...
bool ContactValidator::validatePhone(const string& phone) {
    ...
    if (digits.size() == 11 && (digits[0] == '7' || digits[0] == '8')) return true;

    if (digits.size() == 10 && digits[0] == '9') return true;
